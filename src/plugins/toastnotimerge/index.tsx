...
function stringToList(str: string): string[] {
    return str.trim() ? str.replace(/\s/g, "").split(",") : [];
}


function limitMessageLength(body: string, hasAttachments: boolean): string {
    const maxLength = hasAttachments ? 30 : 165;
...
export default definePlugin({
    ...
    settings,
    flux: {
        async MESSAGE_CREATE({ message }: { message: Message, channel: Channel; }) {

            const channel: Channel = ChannelStore.getChannel(message.channel_id);
            const currentUser = UserStore.getCurrentUser();
            const id = ChannelStore.getDMUserIds();
            ...
            switch (message.type) {
                ...
                case MessageTypes.CHANNEL_RECIPIENT_ADD: {
                    ...
                    const targetUser = UserStore.getUser((message.mentions[0] as any)?.id);
