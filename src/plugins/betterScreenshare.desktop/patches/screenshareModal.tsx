...
export function replacedScreenshareModalSettingsContentType(oldType: (...args: any[]) => any, thisContext: any, functionArguments: any) {
    ...
    if (hideDefaultSettings) {
        // Ensure oldTypeResult.props.children is an array and use optional chaining to safely filter
        oldTypeResult.props.children = Array.isArray(oldTypeResult.props.children)
            ? oldTypeResult.props.children.filter(c => !c?.props?.selectedFPS)
            : [];
    }

    // Safely push ReplacedStreamSettings component if oldTypeResult.props.children is an array
    if (Array.isArray(oldTypeResult.props.children)) {
